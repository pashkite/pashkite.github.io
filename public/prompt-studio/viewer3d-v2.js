...
const camera = new THREE.PerspectiveCamera(45, 1, 0.03, 80);
const baseTarget = new THREE.Vector3(0, 1.02, 0);

scene.add(new THREE.HemisphereLight(0xe9efff, 0x20242e, 2.4));
const key = new THREE.DirectionalLight(0xffffff, 3.5);
key.position.set(-3.2, 6, -4.3);
key.castShadow = true;
...
scene.add(key);
const rim = new THREE.DirectionalLight(0x9eb5ff, 2.0);
rim.position.set(4, 3, 5);
scene.add(rim);
const fill = new THREE.DirectionalLight(0xffc49b, 0.8);
fill.position.set(2, 2.4, -5);
scene.add(fill);

const floor = new THREE.Mesh(
  new THREE.CircleGeometry(5.8, 96),
  new THREE.MeshStandardMaterial({ color: 0x11151d, roughness: 0.92, metalness: 0.01 }),
);
...
function addDirectionArrow(dir, color) {
  scene.add(new THREE.ArrowHelper(dir, new THREE.Vector3(0, 0.028, 0), 2.05, color, 0.26, 0.15));
}
...
function groundedCameraHeight(elevation) {
  const e = THREE.MathUtils.clamp(elevation, -70, 70);
  if (e < 0) {
    const t = Math.pow((-e) / 70, 0.8);
    return THREE.MathUtils.lerp(baseTarget.y, 0.04, t);
  }
  const t = Math.pow(e / 70, 0.9);
  return THREE.MathUtils.lerp(baseTarget.y, 4.0, t);
}

function updateCamera() {
  const s = cameraState();
  const theta = THREE.MathUtils.degToRad(s.az);
  const horizontal = 0.32 + THREE.MathUtils.clamp(s.dist, 0.6, 7) * 0.78;
  const y = groundedCameraHeight(s.el);
  ...
  let lookY = baseTarget.y;
  if (s.el <= -55) lookY = 1.32;
  else if (s.el <= -35) lookY = 1.18;
  else if (s.el >= 55) lookY = 0.72;
  else if (s.el >= 35) lookY = 0.86;
    ...
    loader.load(
      ...
      (event) => {
        ...
        if (event.lengthComputable && event.total > 0) {
          ...
          statusEl.textContent = `${label} 불러오는 중… ${(event.loaded / 1024 / 1024).toFixed(1)}MB`;
        }
      },
      reject,
    );
...
async function loadModel(keyName) {
  ...
  try {
    ...
    if (statusEl) statusEl.textContent = `${def.label} · 로컬 모델`;
    requestRender();
  } catch (err) {
